...
void load_filtered_words(int testcase) {
    char inputFile[256];
    snprintf(inputFile, sizeof(inputFile), "testcase_%d/input.txt", testcase);  // Construct the path to input.txt
...
int count_violations(const char *message) {
    ...
    // Check for filtered words (case-insensitive)
...
void read_input_file(int testcase, int *mod_key, int *threshold) {
    ...
    fscanf(file, "%d %d %d %d %d", &num_groups, &val_key, &app_key, mod_key, threshold);  // Read the relevant fields
...
int main(int argc, char *argv[]) {
    ...
    // Load filtered words from "filtered_words.txt"
